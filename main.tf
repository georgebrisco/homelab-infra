...
# =============================================================================
# Cloudflare tunnel — Bright Pelican (florida_ai)
# =============================================================================
...
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "florida_ai" {
  ...
  config {
    ingress_rule {
      hostname = var.brightpelican_domain
      service  = "http://localhost:80"
    }
    ingress_rule {
      hostname = "www.${var.brightpelican_domain}"
      service  = "http://localhost:80"
    }
    ingress_rule {
      service = "http_status:404"
...
resource "cloudflare_record" "brightpelican_apex" {
  zone_id = var.brightpelican_zone_id
  name    = var.brightpelican_domain
  type    = "CNAME"
  content = "${cloudflare_zero_trust_tunnel_cloudflared.florida_ai.id}.cfargotunnel.com"
  proxied = true
  comment = "bright pelican tunnel"
}

resource "cloudflare_record" "brightpelican_www" {
  zone_id = var.brightpelican_zone_id
  name    = "www"
  type    = "CNAME"
  content = "${cloudflare_zero_trust_tunnel_cloudflared.florida_ai.id}.cfargotunnel.com"
