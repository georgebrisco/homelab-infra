...
| openclaw | 118 | openclaw | .35 | openclaw | OpenClaw AI gateway (WhatsApp/Signal) |
| florida_ai | 918 | florida-ai | .36 | florida_ai, tunnel | Florida AI website (NOT YET DEPLOYED) |
...
AI messaging gateway running on 192.168.50.35. Connects WhatsApp, Signal, and other messaging channels to an LLM backend (currently Anthropic Claude Sonnet 4.6).

**Stack**: OpenClaw gateway (Node.js) bound to 127.0.0.1:18789, fronted by nginx on :443 with self-signed TLS (expires 2036). HTTP :80 redirects to HTTPS. Device auth enabled, trusted proxies restricted to loopback.

**Dashboard**: https://192.168.50.35/ (self-signed cert)
**Auth token**: `6975be834591ed017516a9c8cf29bef663b5549986b2cdde`
**Config**: `/root/.openclaw/openclaw.json` on the container
**Service**: `systemctl --user status openclaw-gateway.service` (user-level systemd, linger enabled)

**Channels**: WhatsApp (linked, connected), Signal (configured, needs `signal-cli register`)

**Ansible role** (`ansible-openclaw/`): Installs Node.js, OpenClaw, nginx TLS proxy, signal-cli. Spec: 2 cores, 4GB RAM, 20GB disk, unprivileged, nesting=1.

**Pending**: Complete Signal registration — SSH to .35, run `signal-cli link -n "OpenClaw Homelab"` and scan QR.

## Florida AI (NOT YET DEPLOYED)

AI consultancy website planned as a React/Vite static site with Cloudflare tunnel.

**Container**: CT 918 at 192.168.50.36 (defined in Terraform but not yet created)
**Site source**: `florida-ai-site/` in this repo (React/Vite, 4 pages: home, services, about, contact)
**Ansible role**: `ansible-florida-ai/` (nginx-svelte role)
**Terraform**: Container definition in `main.tf` + Cloudflare tunnel + DNS in `florida-ai-site/terraform-additions.tf`
**Makefile**: `make florida-ai`

**Needs**: Domain purchased, `terraform apply` to create container, DHCP reservation for .36 added to router config.

## Portfolio Tracker (devbox at 192.168.50.23)

Python app at `/srv/portfolio/` on the devbox container. Tracks stock prices and sends P&L emails.

**Git remote**: `ssh://root@192.168.50.23/srv/git/portfolio-tracker.git`
**Post-receive hook**: Pulls to `/srv/portfolio` on push to main; restarts portfolio-prices if `stock_prices.py` changed.

**Schedule** (TZ=America/New_York — DST-safe):

| Cron | Job |
|------|-----|
| 30 8 * * 1-5 | Start portfolio-prices service (pre-market) |
| 5 9-15 * * 1-5 | Hourly P&L email (9:05 AM – 3:05 PM ET) |
| 30 16 * * 1-5 | EOD close email (4:30 PM ET) |

Service exits cleanly at 23:58 ET each day; 8:30 AM cron restarts it.
...
| florida_ai | florida-ai (.36) | TBD (domain not purchased) | http://localhost:80 |
...
- **containers** → all 20 container hostnames (children: k3s_cluster, tunnel_hosts)
